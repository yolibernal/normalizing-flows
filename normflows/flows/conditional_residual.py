...
class ConditionalResidual(Flow):
    """
    ...
    def __init__(
        ...
        self.iresblock = ConditionaliResBlock(
            net,
            n_samples=n_samples,
    ...
    def forward(self, z, context=None):
        if self.reverse:
            z, log_det = self.iresblock.inverse(z, 0, context=context)
        else:
            z, log_det = self.iresblock.forward(z, 0, context=context)
...
class ConditionaliResBlock(nn.Module):
    ...
    def forward(self, x, logpx=None, context=None):
        if logpx is None:
            y = x + self.nnet(x, context=context)
            return y
        else:
            g, logdetgrad = self._logdetgrad(x, context=context)
            return x + g, logpx - logdetgrad

    def inverse(self, y, logpy=None, context=None):
        x = self._inverse_fixed_point(y, context=context)
        if logpy is None:
            return x
        else:
            return x, logpy + self._logdetgrad(x, context=context)[1]

    def _inverse_fixed_point(self, y, atol=1e-5, rtol=1e-5, context=None):
        x, x_prev = y - self.nnet(y, context=context), y
        i = 0
        tol = atol + y.abs() * rtol
        while not torch.all((x - x_prev) ** 2 / tol < 1):
            x, x_prev = y - self.nnet(x, context=context), x
            i += 1
            if i > 1000:
                break
        return x
